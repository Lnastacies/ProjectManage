...
public class DailyServiceImpl implements DailyService {
	...
	public void dailyToProjectWeekly(Daily weekly, int[] lineNumberArray) {
		...
		int workContentLineNum = 1;
		if(StringUtils.isNotEmpty(weekly.getDailyContent())){
			//计算当前日报内容所占行数
			workContentLineNum = StringUtil.stringByteRowNum(weekly.getDailyContent(),lineNumberArray[0]);
			weekly.setLineNumber(workContentLineNum);
		}else{
			weekly.setLineNumber(workContentLineNum);
		}
		...
		if(StringUtils.isNotEmpty(weekly.getProblem())){
			//计算问题内容所占行数
			int ProblemlineNum = StringUtil.stringByteRowNum(weekly.getProblem(),lineNumberArray[1]);
			if(workContentLineNum>=ProblemlineNum){
				weekly.setLineNumber(workContentLineNum);
			}else{
				weekly.setLineNumber(ProblemlineNum);
			}
		...
		if(StringUtils.isNotEmpty(weekly.getNextWorkPlan())){
			//计算下周计划所占行数
			int planLineNum = StringUtil.stringByteRowNum(weekly.getNextWorkPlan(),lineNumberArray[2]);
			weekly.setPlanLineNumber(planLineNum);
		}
	}
