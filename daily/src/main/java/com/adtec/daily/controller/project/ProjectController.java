...
public class ProjectController {
    ...
    public Msg getProjectByUser(@RequestParam(value = "pn", defaultValue = "1") Integer pn, Model model, @RequestParam String userId) {
        ...
        // 1.管理端查询所有项目信息
        // 根据
        List<Project> pros = projectService.getProjectByUser(userId);
        PageInfo page = new PageInfo(pros, 5);
        return Msg.success().add("pageInfo", page);
    }
    ...
    @ResponseBody
    public Msg getProjectUserByProjectId(@RequestParam(value = "pn", defaultValue = "1") Integer pn, @RequestParam("projectId") int projectId,@RequestParam("roleType") String roleType) {
        PageHelper.startPage(pn, 5);
        int companyId =0;
        List<User> pros = userService.getAllUserByProjectIdAndCompanyId(projectId,companyId,roleType);
        PageInfo page = new PageInfo(pros, 5);
        return Msg.success().add("pageInfo", page);
    }
