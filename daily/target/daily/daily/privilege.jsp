...
<head>
    ...
    <link
            ...
            rel="stylesheet">
    <link
            href="${APP_PATH }/static/jequry.view/jquery.treeview.css"
            rel="stylesheet">
    <script
    ...
    <script src="${APP_PATH}/static/jequry.view/jquery.treeview.js"></script>
</head>
<body>
<!-- 员工修改的模态框 -->
...
<script type="text/javascript">
    ...
    function build_roles_table(result){
        ...
        $.each(roles,function(index,item){
            ...
            var createRoleId = $("<td></td>").append(item.createRoleId);
            var updateTime = $("<td></td>").append(item.updateTime==null?"":new Date(item.updateTime).Format("yyyy-MM-dd"));
            var updateRoleId = $("<td></td>").append(item.updateRoleId);
    ...
    function validate_add_form() {
        ...
        if (!regName.test(roleName)) {
            ...
            show_validate_msg("#roleName_add_input", "error", "用户名可以是2-10位中文或者6-16位英文和数字的组合");
            return false;
        } else {
    ...
    $("#role_update_btn").click(function(){
        ...
        //2、发送ajax请求保存更新的员工数据
        $.ajax({
            url:"${APP_PATH}/role/"+$(this).attr("edit-id"),
            type:"PUT",
    ...
    $("#role_delete_all_btn").click(function(){
        ...
        $.each($(".check_item:checked"),function(){
            ...
            //组装员工id字符串
            del_idstr += $(this).parents("tr").find("td:eq(7)").text()+"-";
        });
        //去除roleNames多余的,
